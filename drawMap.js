...
function initMap() {
  ...
  function CenterControl(controlDiv, map) {
    ...
    edit_ui_btn.addEventListener('click', function () {
      is_edit = true;
      document.querySelector('#form-title').innerHTML = 'Edit ' + active_polygon.name;
  ...
  function PolygonControl(controlDiv, map) {
    ...
    save_btn.addEventListener('click', function () {
      if (document.querySelector('#map-color').value.trim() != '' && document.querySelector('#polygon-name').value.trim() != '') {
        ...
        if (is_edit && active_polygon) {
          ...
          active_polygon.setOptions(getNewPolygonOptions(active_polygon.index, polygon_color, active_polygon.id, polygon_name, false));
          processPolygonLabel(map, active_polygon);
        } else {
          console.log('add');
          drawingManager.setOptions({
            polygonOptions: getNewPolygonOptions(null, polygon_color, null, polygon_name),
          });
        }
        document.querySelector('.polygon-info-wrapper').style.display = 'none';
      }
  ...
  function processPolygonLabel(map, polygon) {
    ...
    let center_coords = turf.getCoords(turf.centerOfMass(turf.polygon(checkLastCoords(cur_coords))));
      ...
      for (let i = 0; i < all_polygons.length; i++) {
        if (i !== active_polygon.index) {
          ...
          if (intersection_area) {
            ...
            //If no other polygons affected from updates then just reposition to the center
            processPolygonLabel(map, active_polygon);
          }
        }
      }
